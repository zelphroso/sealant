        ...
        for line in lines[1:]:
            ...
            try:
                rule = {
                    ...
                    "tide_enabled":     int(parts[14]) if len(parts) > 14 else 0,
                    "tide_days":        int(parts[15]) if len(parts) > 15 else 0,
                    "tide_hour_start":  int(parts[16]) if len(parts) > 16 else 0,
...
def draw_rules(win, rules, scroll, show_ipv6=False):
    ...
    header = f"{'ID':<4} {'NAME':<24} {'FLOE':<12} {'ACTION':<20} {'PROTO':<8} {'IFACE':<16} {'PORT':<8} {'HITS':<10} {'BYTES':<10} {'TIDE'}"
    ...
    for r in visible[scroll:]:
        ...
        tide = tide_str(r)
        line = f"{r['id']:<4} {display_name:<24} {floe:<12} {action:<20} {proto:<8} {iface:<16} {port:<8} {hits:<10} {bytesh:<10} {tide}"
